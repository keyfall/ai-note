...
- 引入cot提示，让模型给出判断过程和答案的解释
	- “客服是否使用了礼貌用语？请详细说明你的判断过程。”   
- 引入Few-Shot学习解决不同客服的表达形式不同，模型难以通过单一提示进行解决，模型可能对提示未见过,给一些正常的案例，给模型一个模板
	- `以下对话中，客服使用了礼貌用语：   - 客服说：‘请问，您遇到的问题是无法登录吗？’    - 客服说：‘麻烦您提供一下订单编号，谢谢！’       请判断接下来的对话中，客服是否使用了礼貌用语。”   
- 增加强调符号，提供角色定位
...
- k-meadns++:因为k-means初始随机选取k中心点，所以导致收敛很慢
	...
	|**算法**|**原理流程**|**优点**|**缺点**|**使用场景**|
	|---|---|---|---|---|
	|**层次聚类**|**凝聚型**：<br>1. 每个数据点初始化为一个簇。<br>2. 计算所有簇间的距离（如单链接，全链接，平均链接）。<br>3. 合并最近的两个簇。<br>4. 更新距离矩阵。<br>5. 重复步骤2-4，直到只剩下一个簇或达到停止条件。<br><br>**分裂型**：<br>1. 所有数据点初始化为一个簇。<br>2. 使用某种规则（如最大距离）将当前簇分成两部分。<br>3. 对每个新簇递归分割，直至满足停止条件。|不需要预先指定簇数；结果以树状图形式展示，易于理解和解释。|高计算复杂度；对噪声敏感；一旦合并/分裂无法撤销。|生物信息学、市场细分研究、社交网络分析等。|
	|**K均值聚类**|1. 随机选择k个中心点。<br>2. 将每个数据点分配到最近的中心点所在的簇。<br>3. 根据簇内数据点重新计算每个簇的中心点。<br>4. 重复步骤2-3，直到簇中心不再变化或达到最大迭代次数。|简单高效；适用于大规模数据集；结果易于解释。|需要预先指定k值；对初始中心点敏感；易陷入局部最优解；对非凸形状簇效果较差。|大规模数据集分析、图像压缩、客户分群等。|
	|**均值漂移聚类**|1. 将每个点作为初始中心点<br>2.对于每个中心点，定义一个半径为 h的窗口（也称为核），然后计算该窗口内所有点相对于当前中心点的偏移量（即漂移）。这个偏移量是基于核加权平均位置计算出来的，其中靠近中心点的数据点具有更大的权重。<br>3. 将当前中心点移动到漂移的位置<br>4. 不断重复计算漂移并更新中心点的过程，直到中心点的位置变化小于某个设定的阈值或者达到最大迭代次数为止。<br>5. 一旦所有的中心点都收敛，根据距离将原始数据集中的点分配给最近的收敛中心点所代表的簇。|不需要预先指定簇数；能够处理任意形状的簇；对异常值不敏感。|收敛速度慢；对带宽参数敏感；计算复杂度高。|图像分割、对象跟踪、模式识别等。|
	|**谱聚类**|1. 构造相似度矩阵（如高斯核函数）。<br>2. 计算拉普拉斯矩阵及其特征值分解。<br>3. 取前k个最小特征值对应的特征向量，构建低维表示。<br>4. 对低维表示进行K均值聚类。<br>5. 输出最终簇划分。|能够识别非凸形状的簇；对数据分布不均匀的情况鲁棒性好；适合复杂结构数据。|计算复杂度高；对相似度矩阵和参数（如σ）敏感；不适用于超大规模数据集。|社交网络分析、图像分割、生物信息学等。|
	|**模糊C均值**|1. 初始化隶属度矩阵（每个样本属于每个簇的概率）。<br>2. 根据隶属度矩阵更新聚类中心。<br>3. 根据新中心更新隶属度矩阵。<br>4. 重复步骤2-3，直到隶属度矩阵收敛或达到最大迭代次数。|样本可以属于多个簇；适合处理重叠群集；提供更灵活的聚类结果。|对噪声和异常值敏感；计算复杂度较高；需要预先指定簇数。|数据挖掘、模式识别中涉及模糊分类的任务，如医学图像分析。|
	|**EM（期望最大化）**|1. 初始化模型参数（如均值、协方差矩阵）。<br>2. **E步**：根据当前参数估计隐变量的后验概率（即每个样本属于每个簇的概率）。<br>3. **M步**：最大化似然函数，更新模型参数。<br>4. 重复E-M步，直到参数收敛或达到最大迭代次数。|可用于存在未观测变量的问题；适用于混合模型；能处理不完全数据。|可能会收敛到局部最优解；对初始参数敏感；计算复杂度较高。|统计推断、机器学习中的参数估计问题，如高斯混合模型。|

| 方法       | 簇间距离定义    | 优点               | 缺点                 |
| -------- | --------- | ---------------- | ------------------ |
| **单链接**  | 最近点距离     | 处理非球形分布；对噪声不敏感   | 易产生链式效应；可能导致过长簇边界  |
| **全链接**  | 最远点距离     | 生成紧凑簇；不易出现链式效应   | 对噪声敏感；难以处理非球形分布    |
| **平均链接** | 所有点对的平均距离 | 平衡单链接和全链接；对噪声不敏感 | 计算复杂度高；可能难以处理复杂簇结构 |
##### 模糊c均值例子：
![[Pasted image 20250426184444.png]]
#### 数据集图片有倾斜，光亮不好
-  对于倾斜的图片：
